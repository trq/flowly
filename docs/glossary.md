...
| **Onboarding Agent** | A dedicated AI SDK `ToolLoopAgent` used to drive multi-step onboarding flows (for example budget setup). It orchestrates tool calls and can stream structured UI specs in chat. |
| **UI Spec (`json-render`)** | A JSON description of UI elements rendered with `@json-render/react`. In chat, these specs are presentational projections and not canonical persisted state. |
| **Event** | A fact about something that happened, delivered from backend to frontend via the SSE stream. Events are the canonical mechanism for pushing state changes to the client. Follows the envelope format: `{ id, channel, type, payload, sentAt }`. |
